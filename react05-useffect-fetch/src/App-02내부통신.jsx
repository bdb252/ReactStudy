...
const GlobalTop=(props) => {
  ...
  /** 목록의 데이터로 사용할 state생성. 초기값은 빈 배열로 설정.
  ...
  */
  ...
  useEffect(function(){
    ...
    fetch('./json/myData.json')
      .then((result)=>{
        /** 요청에 성공하면 json파일의 데이터가 매개변수를 통해 반환된다. 
         반환된 데이터는 Text형식이므로 JSON포맥으로 변환 후 반환한다. */
        return result.json();
      })
      .then((json)=>{
    ...
    return() => {
      /** 컴포넌트가 언마운트될때 실행할 코드가 있다면 이 부분에 작성하면 된다. 
      이 부분은 두번째 렌더링부터 동작한다. */ 
      console.log('#Life', 'LifeGood==>4.useEffect실행2');
    }
  }, []);
  /** useEffect의 두번째 인자인 의존성배열에 빈 배열을 추가한다. 
   이렇게 하면 최초 한번만 실행되고, 그 이상 실행되지 않는다. 
   만약 의존성 배열을 생략하면 무한히 로딩되는 현상이 발생한다. 
   useEffect내부에서 스테이트를 변경하는 코드가 있기 때문이다. 
   
   따라서 useEffect는 모든 렌더링 후마다 실행된다.
   빈 배열은 의존성이 없다는 뜻이라서 처음 한번만 실행되고
   setMyList로 상태가 바뀌어도 의존성 배열이 변하지 않았으므로 useEffect 를 다시 호출하지 않는다. 
   */ 
  var listTag = [];
    ...
    <div>
      ...
      <GlobalTop myLinkClick={(num)=>{
        ...
        fetch('./json/dto'+num+'.json')
          ...
          .then((json)=>{
            console.log('결과', json);
            // JSON 파일의 내용을 통해 스테이트를 변경하고 리렌더링한다. 
            setMyResult(json);
          });
      }}></GlobalTop>
