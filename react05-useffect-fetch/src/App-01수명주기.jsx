...
function LifeGood(props) {
  /*
  이 컴포넌트에서 제일 먼저 실행되는 코드. 즉 렌더링 전에 실행할 코드가 있다면
  이 부분에 작성한다. 
  */ 
  ...
  /**
  useEffect()는 컴포넌트가 렌더링된 후 실행된다. 
  첫 실행에서는 마운트만 되고, 두번째 실행부터 언마운트, 마운트 순으로 동작한다.  
  (컴포넌트를 통해 생성된 UI를 웹브라우저에 출력하는 것을 마운트, 반대로
  제거하는 것을 언마운트라고 한다.)
  */ 
  useEffect(function(){
  ...
  // }, [myCount]); //3.의존성배열에 스테이트변수 할당
  /**
  useEffect의 두번째 인자인 의존성 배열의 유무에 따른 설명
  1.2개의 버튼을 누를때마다 useEffct가 실행된다. 즉 리렌더링이 될 때마다 
    실행된다. 
  ...
  */ 
  ...
  return (
    <div>
      ...
      <p>난수 : {myRandomNum}</p>
      <p>카운트 : {myCount}</p>
      {/* 버튼을 누를 때마다 난수를 생성하거나, 카운트를 증가시켜 스테이트를 
        변경한다. 이 변경은 새로운 렌더링으로 이어진다.  */}
