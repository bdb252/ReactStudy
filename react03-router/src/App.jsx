/**
react-router-dom으로부터 입포트한 컴포넌트와 관련 훅 */ 
...
const TopNavi = () =>{
  ...
  단 a태그에 preventDefault()가 적용된 형태로 화면의 깜빡입 없이 페이지
  이동을 할 수 있다. 또한 링크를 클릭했을 때 active라는 클래스 속성을
  자동으로 추가해준다. 
  */ 
  return (
    <nav>
      ...
      <a href="/aTag" onClick={(e)=>{
        e.preventDefault();}}>A태그</a>&nbsp;
        {/* Link 컴포넌트는 NavList와 기능은 동일하지만, active 속성값이
          부여되지 않는다.  */}
...
/**
localhost/intro 경로가 요쳥될 때 Outlet 컴포넌트 위치에 삽입되어 렌더링된다. 
...
/**
localhost/intro/router 경로가 요청되었을 때 Outlet 컴포넌트 부분에 삽입되어 렌더링된다.  

useLocation 훅
  : React Router를 통해 라우팅된 페이지에서 현재 URL(경로)과 관련된 정보를 얻는데 사용된다. 
    ...
    <div>
      <ul>
        ...
        */}
        ...
        {/* 버튼에 함수 연결시 이벤트리스너에 함수명만 붙여주면 된다.  */}
