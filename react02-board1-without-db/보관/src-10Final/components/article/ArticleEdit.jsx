...
/**
수정 페이지를 구성하기 위해 기존 데이터를 프롭스로 받은 후 input의 value
속성값으로 설정한다. 
하지만 이 경우 input이 readonly속성으로 렌더링되어 기존의 내용을 수정할 수 없게 된다.
...
function ArticleEdit(props){
  ...
  input의 개수만큼 state를 생성한다. 프롭스로 전달된 데이터를 state에 저장한 
  후 변환함수까지 정의한다.
      ...
      <form onSubmit={(event)=>{
        ...
        <table id="boardTable">
          ...
          <tr>
            ...
            input과 동일하게 value속성을 사용한다.  */}
            <td><input type="text" name="contents" value={contents} onChange={(event)=>{
